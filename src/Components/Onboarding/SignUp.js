...
import CountryCodeSelector from "./CountryCodeSelector"; // Assuming this component exists

function SignUp() {
  const [name, setName] = useState("");
  const [business_name, setBusiness_name] = useState(""); // Business name state
  ...
  const handleSubmit = async (e) => {
    ...
    if (!name || !business_name  || !password || !confirmPassword || !phone) {
    ...
    const data = {
      name,
      business_name, // Added businessName in the data
      mobile: `${selectedCountryCode}${phone}`, // Combine country code with phone number
      password,
      role: "business",
    };
    ...
    try {
      ...
      // Display a success toast on successful API response
      toast.success("Registration successful!");
      navigate("/SendOtp"); // Redirect to SendOtp component
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="mb-3">
                    <input
                      ...
                      value={business_name}
                      onChange={(e) => setBusiness_name(e.target.value)} // Handling business name input
                    />
                  </div>
                  {/* <div className="mb-3">
                    <select
